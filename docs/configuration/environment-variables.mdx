...
Set `OPENSRE_DISABLE_CREDENTIAL_FALLBACK=1` if you would rather setup fail than
have a credential written to disk.
...
| `OPENSRE_DISABLE_CREDENTIAL_FALLBACK` | `0` | Refuse the on-disk fallback store, so setup fails instead when no keychain works | `config/secrets` |
| `OPENSRE_CREDENTIAL_FALLBACK_PATH` | `~/.opensre/credentials.json` | Where the fallback credential store is written | `config/secrets` |
